...
const FeatureCard = ({ title, description, icon, image, imageAlt, delay = 0, isVisible = false, direction = 'up' }: FeatureCardProps) => {
  ...
  return (
    <div
      className={`group bg-bg-secondary/50 border border-border-custom rounded-xl-custom overflow-hidden transition-all duration-700 ease-out hover:border-brand/50 hover:bg-bg-secondary hover:-translate-y-2 hover:shadow-[0_20px_60px_rgba(108,92,231,0.15)] ${directionClasses[direction]}`}
...
export const FeaturesSection = () => {
  ...
  return (
    <section ref={sectionRef} id="features-section" className="py-24 bg-bg-primary relative overflow-hidden">
      ...
      {/* Subtle rotating orb */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] pointer-events-none animate-rotate-slow opacity-[0.02]">
        <div className="absolute top-0 left-1/2 w-full h-[1px] bg-gradient-to-r from-transparent via-brand to-transparent" />
      </div>

      <div className="max-w-[1280px] mx-auto px-8 relative z-10">
        <div className={`text-center max-w-2xl mx-auto mb-16 transition-all duration-700 ${headerVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
