  ...
  return (
    ...
    >
      ...
      <div className="max-w-[1280px] mx-auto px-8 relative z-10">
        ...
        <div
          className={`relative rounded-2xl overflow-hidden mb-20 border border-border-custom/40 shadow-[0_20px_60px_rgba(0,0,0,0.4)] transition-all duration-700 ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
          {steps.map((step, index) => (
            <div
              key={step.number}
              className={`group flex gap-5 md:gap-6 transition-all duration-700 ${
                isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
              }`}
              style={{ transitionDelay: `${(index + 1) * 150}ms` }}
            >
              {/* Step image */}
              <div className="shrink-0 w-[100px] h-[100px] md:w-[130px] md:h-[130px] rounded-xl overflow-hidden border border-border-custom/50 shadow-lg-custom group-hover:shadow-[0_8px_30px_rgba(0,0,0,0.5)] transition-all duration-500 group-hover:border-brand/40">
                <img
                  src={step.image}
                  alt={step.title}
